...
Форма использует **invisible-режим**: кнопки «Я не робот» нет, проверка запускается при отправке; challenge появляется только если Yandex посчитает запрос подозрительным. В углу страницы — небольшой shield Yandex (требование сервиса).
...
2. Напиши в группе любое сообщение (или `/start`, или `@имя_бота тест`).
3. Узнай `chat_id`: `docker exec aivids_app php artisan telegram:discover-chats`
4. Скопируй id группы в `TELEGRAM_CHAT_ID`, перезапусти config cache на проде.
